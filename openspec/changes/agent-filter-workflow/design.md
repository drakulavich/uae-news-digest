...
- This change does not alter the existing Digest ranking, Signal filter, topics
  config, Core API, or human-readable rendering.
...
```json
{
  ...
  "items": [{ "id": "…", "title": "…", "tier": "impact" }],
  "next": { "command": "agent commit", "runId": "…" }
}
```

Sana applies `instructions` to `items`, then invokes `agent commit --run-id …`
with zero or more `--keep` values. A two-step protocol is required because the
CLI has no LLM dependency and therefore cannot make the filtering decision
itself.
...
Collection stores the candidate IDs, their Seen-item keys, the selected state
file, and an expiry timestamp in a private cache location under
...
On success, it records every candidate as reviewed: the external model has read
and rejected the noise, so re-presenting it wastes later runs. It returns the
kept Items as the model's final machine-readable Digest. A successful commit
deletes its record; stale records are removed during collection and commit.
...
2. Document the JSON contract, persistent rules file, and two calls in help,
   README, and `manifest`.
3. Add deterministic CLI tests for collection, rule composition, commit, expiry,
...
- Confirm whether `agent collect` needs first-class topics-mode support in the
  initial release. This proposal specifies the region-style candidate Digest;
  extending it to a Topics config changes the result shape and needs an explicit
